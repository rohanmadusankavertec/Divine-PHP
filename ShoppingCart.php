    ...
    <head>
        ...
        <script type="text/javascript">
            ...
            function DeleteSessionCart(id) {
                ...
                xmlHttp.open("POST", "src/Cart.php?action=deletesession&id=" + id, true);
                xmlHttp.send();
            }
        </script>
                            ...
                            if (isset($_SESSION['user_type'])) {
                                ...
                                if (isset($_SESSION['session_cart'])) {
                                    ...
                                    for ($index = 0; $index < sizeof($ci); $index++) {
                                        ...
                                        include_once './src/DBConnection.php';
                                        $sql = "SELECT p.id,p.name,p.img,p.price FROM product p where p.id='" . $parr[0] . "'";
                                        $result = $conn->query($sql);
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-xs-12 col-sm-2 col-md-2">
                                                        ...
                                                        <div class="input-group input-group-sm">
                                                            <span class="input-group-addon">Qty.</span>
                                                            <span>
                                                                <input type="text" disabled value="<?php echo $parr[1]; ?>" maxlength="3" id="qty<?php echo $row["id"]; ?>" class="form-control input-sm" style="width:50px;text-align: center; font-weight: bold; font-size: 1em;">
                                                                <!--<a onclick="UpdateCart(<?php // echo $row["id"];   ?>)" class="btn btn-link btn-sm">Update</a></span>-->
                                                                </div>
                                                                </div>
                                                                <div class="col-xs-12 col-sm-2 col-md-2">
                                                                ...
                                                                <div class="col-xs-12 col-sm-2 col-md-2">
                                                                    <br>
                                                                    <span class="visible-sm visible-md visible-lg"></span>
                                                                    <div class="hidden-xs">Rs. <?php echo (floatval($row["price"]) * floatval($parr[1])) ?></div>
                                                                    <div class="visible-xs">Item Total: Rs.<?php echo (floatval($row["price"]) * floatval($parr[1])) ?></div>
                                                                </div>
                                                                <div class="col-xs-12 col-sm-1 col-md-1" style="padding-left: 0; padding-right: 0;">
                                                                    <a class="btn btn-link btn-sm" onclick="DeleteSessionCart(<?php echo $row["id"]; ?>)"><span class="glyphicon glyphicon-remove-sign"></span><br>Remove</a>
                                                                </div>
